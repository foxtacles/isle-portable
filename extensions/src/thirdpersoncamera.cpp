...
ThirdPersonCameraExt::ThirdPersonCameraExt()
	: m_animator(CharacterAnimatorConfig{/*.saveEmoteTransform=*/true}),
	  m_enabled(false), m_active(false), m_pendingWorldTransition(false), m_playerROI(nullptr)
{
}

void ThirdPersonCameraExt::Enable()
{
	m_enabled = true;
	ReinitForCharacter();
}

void ThirdPersonCameraExt::Disable()
{
	m_enabled = false;
...
void ThirdPersonCameraExt::OnActorEnter(IslePathActor* p_actor)
{
...
void ThirdPersonCameraExt::OnActorExit(IslePathActor* p_actor)
{
	if (!m_enabled) {
		return;
	}
...
void ThirdPersonCameraExt::OnCamAnimEnd(LegoPathActor* p_actor)
{
	m_pendingWorldTransition = false;
...
void ThirdPersonCameraExt::Tick(float p_deltaTime)
{
...
void ThirdPersonCameraExt::SetWalkAnimId(uint8_t p_walkAnimId)
{
	m_animator.SetWalkAnimId(p_walkAnimId, m_active ? m_playerROI : nullptr);
}

void ThirdPersonCameraExt::SetIdleAnimId(uint8_t p_idleAnimId)
{
	m_animator.SetIdleAnimId(p_idleAnimId, m_active ? m_playerROI : nullptr);
}

bool ThirdPersonCameraExt::IsInMultiPartEmote() const
{
	return m_animator.IsInMultiPartEmote();
}

int8_t ThirdPersonCameraExt::GetFrozenEmoteId() const
{
	return m_animator.GetFrozenEmoteId();
}

void ThirdPersonCameraExt::TriggerEmote(uint8_t p_emoteId)
{
	if (!m_active) {
		return;
	}
...
void ThirdPersonCameraExt::StopClickAnimation()
{
	m_animator.StopClickAnimation();
}

void ThirdPersonCameraExt::OnWorldEnabled(LegoWorld* p_world)
{
	if (!p_world) {
		return;
	}
...
void ThirdPersonCameraExt::OnWorldDisabled(LegoWorld* p_world)
{
	if (!p_world) {
		return;
	}
...
MxBool ThirdPersonCameraExt::HandleCameraRelativeMovement(
...
)
{
	return m_orbit.HandleCameraRelativeMovement(
		p_nav, p_curPos, p_curDir, p_newPos, p_newDir, p_deltaTime,
		m_animator.IsInMultiPartEmote()
	);
}

void ThirdPersonCameraExt::HandleSDLEventImpl(SDL_Event* p_event)
{
	m_input.HandleSDLEvent(p_event, m_orbit, m_active);
}

void ThirdPersonCameraExt::ReinitForCharacter()
{
	LegoPathActor* userActor = UserActor();
	if (!userActor) {
		m_active = false;
...
public:
	TickleAdapter(ThirdPersonCameraExt* p_camera) : m_camera(p_camera) {}
...
ThirdPersonCameraExt* ThirdPersonCameraExt::s_camera = nullptr;
...
{
	if (!s_camera) {
		s_camera = new ThirdPersonCameraExt();
	}
	s_camera->Enable();
}
...
void ThirdPersonCameraExt::OnSDLEvent(SDL_Event* p_event)
{
	...
	if (s_camera->ConsumeAutoDisable()) {
		...
		s_camera->SetOrbitDistance(ThirdPersonCameraExt::MIN_DISTANCE);
		s_camera->Enable();
	}
}
...
MxBool ThirdPersonCameraExt::HandleROIClick(LegoROI* p_rootROI, LegoEventNotificationParam& p_param)
{
	...
	if (effectROI) {
		CharacterCustomizer::PlayClickSound(
			effectROI,
			s_camera->GetCustomizeState(),
			changeType == CHANGE_MOOD
		);
		if (!s_camera->IsInVehicle() && !s_camera->IsInMultiPartEmote()) {
			s_camera->StopClickAnimation();
			MxU32 clickAnimId =
				CharacterCustomizer::PlayClickAnimation(effectROI, s_camera->GetCustomizeState());
...
MxBool ThirdPersonCameraExt::IsClonedCharacter(const char* p_name)
{
	...
	if (s_camera->GetDisplayROI() != nullptr &&
		!SDL_strcasecmp(s_camera->GetDisplayROI()->GetName(), p_name)) {
		return TRUE;
	}
...
ThirdPersonCameraExt* ThirdPersonCameraExt::GetCamera()
{
	return s_camera;
}
