...
using Multiplayer::IsMultiPartEmote;
...
void NetworkManager::BroadcastLocalState()
{
	...
	if (cam) {
		...
		Multiplayer::PackCustomizeState(cam->GetCustomizeState(), msg.customizeData);

		// Encode multi-part emote frozen state
		int8_t frozenId = cam->GetFrozenExtraAnimId();
		if (frozenId >= 0) {
		...
		if (cam->IsExtraAnimBlocking() || cam->IsAnimPlaying()) {
			msg.speed = 0.0f;
		}
	}
...
{
	if (p_emoteId >= Multiplayer::g_emoteAnimCount) {
		return;
	}
...
void NetworkManager::HandleEmote(const EmoteMsg& p_msg)
{
	...
	auto it = m_remotePlayers.find(peerId);
	if (it != m_remotePlayers.end()) {
		it->second->TriggerExtraAnim(p_msg.emoteId);
	}
}
...
void NetworkManager::HandleCustomize(const CustomizeMsg& p_msg)
{
	...
	if (it != m_remotePlayers.end()) {
		if (it->second->GetROI()) {
			...
			if (!it->second->IsMoving() && !it->second->IsExtraAnimBlocking() && !it->second->IsAnimationLocked()) {
