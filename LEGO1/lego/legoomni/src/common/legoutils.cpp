...
MxBool RemoveFromCurrentWorld(const MxAtomId& p_atomId, MxS32 p_id)
{
	...
	auto result =
		Extension<SiLoader>::Call(SI::HandleRemove, SiLoader::StreamObject{p_atomId, p_id}, world).value_or(std::nullopt);
	if (result) {
		return result.value();
	}
...
)
{
	...
	auto result = Extension<SiLoader>::Call(SI::HandleRemove, SiLoader::StreamObject{p_entityAtom, p_entityId}, world)
