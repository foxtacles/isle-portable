...
MxLong Isle::HandleEndAction(MxEndActionNotificationParam& p_param)
{
	...
	default:
		...
		if (result == 0) {
			...
			else if (auto replacedObject = Extension<SiLoader>::Call(SI::ReplacedIn, *p_param.GetAction(), *g_jukeboxScript).value_or(std::nullopt)) {
...
MxLong Isle::HandleControl(LegoControlManagerNotificationParam& p_param)
{
	if (p_param.m_enabledChild == 1) {
		if (Extension<MultiplayerExt>::Call(MP::HandleSkyLightControl, (MxU32) p_param.m_clickedObjectId).value_or(FALSE)) {
			return 1;
		}
